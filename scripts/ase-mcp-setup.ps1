# ase-mcp-setup.ps1
# One-shot setup for the ase-mcp bridge.
#   (default)  VM mode: portproxy + scoped firewall so a Claude Code VM can
#              reach the HTTP MCP on the host. Run as Administrator on the HOST.
#   -Local     same-machine mode: Claude Code and Aseprite on ONE box. Skips
#              portproxy + firewall, points .mcp.json at 127.0.0.1, no admin.
# Same portproxy convention as the other DCC bridges.
...
if (-not $Local) {
    ...
    # Same-machine mode: Claude Code and Aseprite share one box. No cross-machine
    # hop, so no portproxy and no firewall rule (nothing external reaches run_lua).
    # The client connects straight to the loopback the server binds. No admin needed.
    $HostIP = "127.0.0.1"
    Write-Host "[LOCAL] Same-machine mode: no portproxy, no firewall, url = 127.0.0.1:$Port" -ForegroundColor Green

    $serverOk = Test-NetConnection -ComputerName 127.0.0.1 -Port $Port -WarningAction SilentlyContinue
    if ($serverOk.TcpTestSucceeded)  { Write-Host "[OK]   ase-mcp reachable on localhost:$Port" -ForegroundColor Green }
    else                             { Write-Host "[WARN] ase-mcp not responding on localhost:$Port -- start Aseprite with the extension, or run: python server.py" -ForegroundColor Yellow }
}
...
# into the repo-root .mcp.json (gitignored), which Claude Code reads (over the
# shared folder in VM mode, or directly in same-machine mode).
...
Write-Host "[OK]   MCP config written: $writePath (token fingerprint $fingerprint)" -ForegroundColor Green
Write-Host ""
if (-not $Local) {
    ...
    Write-Host "Restart Claude Code in this project to load the ase-mcp server (127.0.0.1)."
}
Write-Host ""
